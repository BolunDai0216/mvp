...
class FrankaReach(BaseTask):
    def __init__(
        ...
        self.hand_pos = torch.zeros(
            (self.num_envs, 3), dtype=torch.float, device=self.device
        )
        self.to_target = torch.zeros(
            (self.num_envs, 3), dtype=torch.float, device=self.device
        )
        ...
        self.goal_pos_init = torch.tensor(
            cfg["env"]["goal_pos_init"], dtype=torch.float, device=self.device
        )
        self.goal_pos_delta = torch.tensor(
            cfg["env"]["goal_pos_delta"], dtype=torch.float, device=self.device
        )
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            goal_actor = self.gym.create_actor(
                env_ptr, goal_asset, goal_start_pose, "goal", self.num_envs + i, 0, 0
            )
            goal_color = gymapi.Vec3(0.466, 0.674, 0.188)
            self.gym.set_rigid_body_color(
                env_ptr, goal_actor, 0, gymapi.MESH_VISUAL_AND_COLLISION, goal_color
            )
        ...
        self.rigid_body_hand_ind = self.gym.find_actor_rigid_body_handle(
            env_ptr, franka_actor, "panda_hand"
        )
        ...
        self.env_goal_ind = self.gym.get_actor_index(
            env_ptr, goal_actor, gymapi.DOMAIN_ENV
        )
    ...
    def reset(self, env_ids):
        ...
        # Goal multi env ids
        goal_multi_env_ids_int32 = self.global_indices[
            env_ids, self.env_goal_ind
        ].flatten()

        # Reset goal pos
        delta_x = torch_rand_float(
            -self.goal_pos_delta[0],
            self.goal_pos_delta[0],
            (len(env_ids), 1),
            device=self.device,
        ).squeeze(dim=1)
        delta_y = torch_rand_float(
            -self.goal_pos_delta[1],
            self.goal_pos_delta[1],
            (len(env_ids), 1),
            device=self.device,
        ).squeeze(dim=1)
        delta_z = torch_rand_float(
            -self.goal_pos_delta[2],
            self.goal_pos_delta[2],
            (len(env_ids), 1),
            device=self.device,
        ).squeeze(dim=1)

        self.root_state_tensor[env_ids, self.env_goal_ind, 0] = (
            self.goal_pos_init[0] + delta_x
        )
        self.root_state_tensor[env_ids, self.env_goal_ind, 1] = (
            self.goal_pos_init[1] + delta_y
        )
        self.root_state_tensor[env_ids, self.env_goal_ind, 2] = (
            self.goal_pos_init[2] + delta_z
        )
...
def compute_franka_reward(
    ...
    goal_bonus_reward = torch.where(
        d <= 0.04, goal_bonus_reward + 0.5, goal_bonus_reward
    )
    ...
    # Total reward
    rewards = (
        goal_dist_reward_scale * goal_dist_reward
